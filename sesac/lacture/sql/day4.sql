...
*/
select name, type from mypokemon where name = 'eevee';
/*
MISSION (2)
캐터피의 공격력과 방어력을 가져와 주세요

*/
select name, attack, defense from mypokemon where name = 'caterpie';
/*MISSION (3)
...
*/
select name, height, weight from mypokemon where height > 0.5 and weight>=6;
/*MISSION (5)
포켓몬 테이블에서 공격력이 50 미만이거나, 방어력이 50 미만인 포켓몬들의 이름을
‘weak_pokemon’이라는 별명으로 가져와 주세요.
*/
select name as ‘weak_pokemon’, attack, defense from mypokemon where attack<50 or defense < 50;
/*MISSION (6)
...
*/
select name, attack, defense from mypokemon where (attack- defense) not between -10 and 10;
/*MISSION (14)
